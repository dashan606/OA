...
public class DepAction implements RequestAware {
	...
	public String deleteDep() {
		df.deleteByLogic(Department.class, new Serializable[] { depId },
				"dep_id", "flag");
		return "deleteDep";
	}
	...
	public String findByPage() {
		int pageNo = 0;
		String pageNo_str = ServletActionContext.getRequest().getParameter(
				"pager.offset");
		if (pageNo_str != null) {
		...
		if (type != null && !type.equals("")) {
			pv = dfr.findByPage(Department.class,
					"from Department d where d.dep_name=?", type, pageNo,
					pageSize);
		} else {
			pv = dfr.findByPage(Department.class, "from Department d ", pageNo,
					pageSize);
		}
		request.put("pv", pv);
		return "findByPage";
	...
	public String execute() throws IOException {
		...
		for(Department dep : subDeps) {
			...
			out.println("<subDep-id>"+dep.getDep_id()+"</subDep-id>");
			out.println("</subDep>");
		}
		out.println("</subDeps>");
		return null;
